...
describe('PedidosService automatic assignment', () => {
  const pedidosRepository = {
    create: jest.fn(),
    save: jest.fn(),
    findAssignmentCandidates: jest.fn(),
  };
  const notificationsService = { notifyDomiciliarioAsignado: jest.fn() };
  const usuariosService = { findOne: jest.fn() };
  ...
  beforeEach(() => {
    ...
    pedidosRepository.save.mockImplementation(async (payload) => ({ ...payload, id: 'pedido-uuid' }));
    usuariosService.findOne.mockResolvedValue({ id: 'domi-old', nombre: 'Domi Old' });
    notificationsService.notifyDomiciliarioAsignado.mockResolvedValue(undefined);
  });

  it('prioriza el domiciliario que lleva mas tiempo sin pedido', async () => {
    pedidosRepository.findAssignmentCandidates.mockResolvedValue([
      { id: 'domi-recent', nombre: 'Domi Recent', lastAssignedAt: '2026-06-16T12:00:00.000Z' },
      { id: 'domi-old', nombre: 'Domi Old', lastAssignedAt: '2026-06-10T12:00:00.000Z' },
    ]);

    await service.createPedidoByAdmin(baseDto, 'admin-uuid');

    expect(pedidosRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ usuarioId: 'domi-old', domiciliarioId: 'domi-old' }),
    );
  });

  it('elige aleatoriamente cuando dos domiciliarios empatan', async () => {
    const randomSpy = jest.spyOn(Math, 'random').mockReturnValue(0.75);
    pedidosRepository.findAssignmentCandidates.mockResolvedValue([
      { id: 'domi-a', nombre: 'Domi A', lastAssignedAt: null },
      { id: 'domi-b', nombre: 'Domi B', lastAssignedAt: null },
    ]);

    await service.createPedidoByAdmin(baseDto, 'admin-uuid');

    expect(pedidosRepository.create).toHaveBeenCalledWith(
      expect.objectContaining({ usuarioId: 'domi-b', domiciliarioId: 'domi-b' }),
    );
